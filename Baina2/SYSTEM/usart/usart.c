...
void uart_init(u32 bound){
  //GPIO端口设置
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOC, ENABLE);	//使能USART1，GPIOA时钟
...
void SendData(uint16_t* data,u8 N)
{
	u8 count_usart;
	for(count_usart=0;count_usart<N;count_usart++)
	{
		USART_SendData(UART5, data[count_usart]);
